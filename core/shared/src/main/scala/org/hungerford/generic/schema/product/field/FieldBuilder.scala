...
import org.hungerford.generic.schema.coproduct.field.{Field, FieldCase}
...
case class FieldBuilder[ T, F, N, Sch, S, E ](
    ...
    private val vs : Set[ Validator[ F ] ] = Set.empty[ Validator[ F ] ],
    private val exs : Seq[ F ] = Nil,
    private val df : Option[ F ] = None,
    ...
    ) : FieldSchemaRebuilder[ T, F, N, Sch, S, E, rb.Builder ] = FieldSchemaRebuilder[ T, F, N, Sch, S, E ]( this )
    def primitive : FieldBuilder[ T, F, N, Primitive[ F ], Unit, E ] = copy( sch = Primitive[ F ]() )
    def singleton[ SN <: TypeName, SV <: Singleton & F ]( value : SV, identifier : TypeName ) : FieldBuilder[ T, F, N, Schema.Aux[ F, SingletonShape[ SV, SN ] ], SingletonShape[ SV, SN ], E ] =
        fromSchema[ SingletonShape[ SV, SN ] ]( SingletonShape( identifier, value ) )
...
object FieldBuilder {
    ...
    def from[ T, F, N <: FieldName, S ]( field : Field.Aux[ T, F, N, S ] ) : FieldBuilder[ T, F, N, S ] =
        FieldBuilder[ T, F, N, S ](
            field.fieldName,
            field.extractor,
            field.schema,
            field.description,
...
case class FieldSchemaRebuilder[ T, F, N, Sch, S, E, Builder ](
    fieldBuilder: FieldBuilder[ T, F, N, Sch, S, E ],
)(
    using
